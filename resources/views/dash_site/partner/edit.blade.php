...
<div class="box box-success">
    ...
    <form role="form" action="{{route('partner.update' , $row->id)}}" method="POST" enctype="multipart/form-data">
      ...
      <div class="box-body">
        ...
        <div class="form-group">
            <label>الموقع</label>
            <textarea  name="website" class="form-control" rows="3" placeholder="النص">{{$row->website}}</textarea>
        </div>
        @error('website')
        <div class="alert alert-danger">{{ $message }}</div>
        @enderror
        <div class="form-group">
            <label>فيسبوك</label>
            <textarea  name="facebook" class="form-control" rows="3" placeholder="النص">{{$row->facebook}}</textarea>
        </div>
        @error('facebook')
        <div class="alert alert-danger">{{ $message }}</div>
        @enderror
        <div class="form-group">
            <label>تويتر</label>
            <textarea  name="twitter" class="form-control" rows="3" placeholder="النص">{{$row->twitter}}</textarea>
        </div>
        @error('twitter')
        <div class="alert alert-danger">{{ $message }}</div>
        @enderror
        <div class="form-group">
            ...
            <textarea  name="instgram" class="form-control" rows="3" placeholder="النص">{{$row->instgram}}</textarea>
        </div>
        @error('instgram')
        <div class="alert alert-danger">{{ $message }}</div>
        @enderror
